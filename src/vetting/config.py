...
@dataclass
class VettingConfig:
    """Configuration holder for the vetting subsystem."""
    ...
    @classmethod
    def load(cls, path: Optional[str] = None) -> "VettingConfig":
        """Load configuration from JSON + environment overrides."""
        default_path = Path(os.getenv("VETTING_CONFIG_PATH", "") or Path.cwd() / "vetting_config.json")
        ...
        internet_enabled = _parse_bool(os.getenv("VETTING_INTERNET_ENABLED"), default=internet_from_file)
        auto_attach_default = _parse_bool(data.get("auto_attach_on_ingest"), default=False)
        auto_attach = _parse_bool(os.getenv("VETTING_AUTO_ATTACH_ON_INGEST"), default=auto_attach_default)
        block_default = _parse_bool(data.get("block_high_risk"), default=False)
        block_high_risk = _parse_bool(os.getenv("VETTING_BLOCK_HIGH_RISK"), default=block_default)

        if os.getenv("VETTING_SIGMOID_A"):
            try:
                sigmoid["a"] = float(os.getenv("VETTING_SIGMOID_A", sigmoid.get("a", 6.0)))
            except ValueError:
                pass
        if os.getenv("VETTING_SIGMOID_B"):
            try:
                sigmoid["b"] = float(os.getenv("VETTING_SIGMOID_B", sigmoid.get("b", 0.50)))
            except ValueError:
                pass

        if os.getenv("VETTING_RISK_HIGH"):
            try:
                risk_thresholds["high"] = float(os.getenv("VETTING_RISK_HIGH", "75"))
            except ValueError:
                pass
        if os.getenv("VETTING_RISK_MEDIUM"):
            try:
                risk_thresholds["medium"] = float(os.getenv("VETTING_RISK_MEDIUM", "45"))
            except ValueError:
                pass

        if os.getenv("VETTING_SEARCH_PROVIDER"):
            search_provider["provider"] = os.getenv("VETTING_SEARCH_PROVIDER")
        if os.getenv("VETTING_SEARCH_API_KEY"):
            search_provider["api_key"] = os.getenv("VETTING_SEARCH_API_KEY")
        if os.getenv("VETTING_SEARCH_ENDPOINT"):
            search_provider["endpoint"] = os.getenv("VETTING_SEARCH_ENDPOINT")
