...
from .models import ToolResult, VettingContext, VettingReport
...
CATEGORY_TOOL_MAP = {
    ...
    "resume": ["resume_vetting", "resume_entity_validation"],
}


class VettingEngine:
    """Runs configured vetting tools and blends their scores."""

    def __init__(
        self,
        config: Optional[VettingConfig] = None,
        tools: Optional[Sequence[VettingTool]] = None,
        search_client: Optional[SearchClient] = None,
    ) -> None:
        self.config = config or VettingConfig.load()
        self.search_client = search_client or self._build_search_client(self.config)
        self._tools: Dict[str, VettingTool] = {tool.name: tool for tool in (tools or self._default_tools())}

    def _build_search_client(self, cfg: VettingConfig) -> SearchClient:
    ...
    def _with_config_override(self, internet_enabled_override: Optional[bool]) -> VettingConfig:
    ...
    def _default_tools(self) -> List[VettingTool]:
        return [
            IntegrityHashTool(),
            MetadataConsistencyTool(),
            TemplateConformanceTool(),
    ...
    def _active_tools(self, doc_type: Optional[str], cfg: Optional[VettingConfig] = None) -> List[VettingTool]:
    ...
    def _risk_level(self, overall_score: float, cfg: Optional[VettingConfig] = None) -> str:
        config = cfg or self.config
    ...
    def _blend_score(self, results: Sequence[ToolResult], cfg: Optional[VettingConfig] = None) -> float:
        config = cfg or self.config
    ...
    def _resolve_tools_for_category(
        self, category: str, doc_type: Optional[str], cfg: VettingConfig
    ) -> List[VettingTool]:
        normalized = category.lower()
        names = CATEGORY_TOOL_MAP.get(normalized)
        tools: List[VettingTool]
        if names:
    ...
    def _run_tools(self, ctx: VettingContext, tools: Sequence[VettingTool]) -> List[ToolResult]:
    ...
    def vet(self, ctx: VettingContext, cfg: Optional[VettingConfig] = None) -> VettingReport:
    ...
    def run_one(
        ...
        *,
        ...
        if not tool:
            raise ValueError(f"Unknown vetting tool '{tool_name}'")
    ...
    def run_category(
        ...
        *,
        ...
        if not tools:
            raise ValueError(f"No vetting tools found for category '{category}'")
    ...
    def run_all(
        ...
        *,
        ...
        return self.vet(ctx, cfg=ctx.config)
...
_DEFAULT_ENGINE = VettingEngine()


def vet_and_attach_metadata(
    text: str,
    metadata: Optional[Dict[str, Any]] = None,
    doc_type: Optional[str] = None,
    raw_bytes: Optional[bytes] = None,
    ...
    Attaches vetting results under metadata["vetting_report"] and returns the metadata.
    The hook never blocks ingestion and respects the VettingConfig toggle.
    """
    ...
    metadata["vetting_report"] = result
    # Backward-compatible alias used by earlier ingestion paths.
    metadata.setdefault("vetting", result)
    return metadata
