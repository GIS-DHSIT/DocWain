...
from src.web.session_history import get_session_history
...
def create_chain(run_id,llm: BaseLanguageModel, retriever: VectorStoreRetriever) -> Runnable:
    ...
    chain = (
        RunnablePassthrough.assign(
            ...
            | retriever.with_config({"run_name": run_id})
            | format_documents
        )
        | prompt
        | llm
    ...
    ).with_config({"run_name": run_id})


async def ask_question(run_id,chain: Runnable, question: str, session_id: str):
    async for event in chain.astream_events(
        ...
        version="v2",
        include_names=["context_retriever", run_id],
    ):
