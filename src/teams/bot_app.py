...
adapter_settings = BotFrameworkAdapterSettings(MICROSOFT_APP_ID, MICROSOFT_APP_PASSWORD) if BOT_CREDENTIALS_CONFIGURED else None
bot_adapter = BotFrameworkAdapter(adapter_settings) if adapter_settings else None
...
def _as_activity(payload: Dict[str, Any]) -> Activity:
    try:
...
class DocWainTeamsBot(TeamsActivityHandler):
    ...
    @staticmethod
    async def _send_safe(turn_context: TurnContext, activity: Activity, log) -> None:
        MicrosoftAppCredentials.trust_service_url(turn_context.activity.service_url)
        try:
            await turn_context.send_activity(activity)
        except ErrorResponseException as exc:
            log.error(
                "Teams outbound unauthorized | serviceUrl=%s channelId=%s recipient_id=%s conversation_id=%s app_id=%s error=%s",
                getattr(turn_context.activity, "service_url", None),
                getattr(turn_context.activity, "channel_id", None),
                getattr(getattr(turn_context.activity, "recipient", None), "id", None),
                getattr(getattr(turn_context.activity, "conversation", None), "id", None),
                MICROSOFT_APP_ID,
                exc,
            )
            log.warning(
                "Connector unauthorized; check Azure Bot AppId/secret, Teams manifest botId, Teams channel enablement, and endpoint routing."
            )
    ...
    async def on_message_activity(self, turn_context: TurnContext):
        ...
        if not question or question.lower() == "help":
...
async def handle_bot_error(turn_context: TurnContext, error: Exception):  # type: ignore[func-returns-value]
...
docwain_teams_bot = DocWainTeamsBot()
