# """
# Enhanced Retrieval System with Multiple Search Strategies - FIXED VERSION
# Fixes: Hybrid search API compatibility, adjacent chunk expansion, document-specific retrieval
# """
#
# import logging
# import numpy as np
...
#
#     def _create_chunk_with_context(self, current_text: str, prev_text: str, next_text: str, section_title: str) -> str:
#         parts = []
...
#         return " ".join(parts)
#
#     def chunk_document(self, text: str, doc_name: str, page_number: int = None) -> List[Tuple[str, ChunkMetadata]]:
...
#                 contextualized_text = self._create_chunk_with_context(chunk_text, prev_text, next_text, current_section)
#
...
#             chunk_text = " ".join([u[0] for u in current_chunk_units])
#             prev_text = chunks[-1][0] if chunks else ""
#             contextualized_text = self._create_chunk_with_context(chunk_text, prev_text, "", current_section)
#
#             metadata = ChunkMetadata(
#                 chunk_id=f"{doc_name}_chunk_{len(chunks)}",
#                 doc_name=doc_name,
...
#
#
# def chunk_text_for_embedding(text: str, doc_name: str) -> List[Tuple[str, dict]]:
#     """Main function for embedding chunking"""
...
#     """
#     Multi-strategy retrieval with FIXED API compatibility
#     """
#
...
#         # Pre-fit on common corpus to avoid single-text fitting issues
#         self._fit_tfidf()
#
#     def _fit_tfidf(self):
...
#
#     def _create_sparse_vector(self, text: str) -> Optional[SparseVector]:
#         """Create sparse vector with proper error handling"""
#         try:
...
#             )
#         except Exception as e:
#             logger.debug(f"Sparse vector creation failed: {e}")
#             return None
#
#     def _build_filter(
#             self,
...
#     ) -> Optional[Filter]:
#         """Build Qdrant filter with proper structure"""
#         conditions = []
#
#         # Profile filter
#         if profile_id:
...
#             return None
#
#         return Filter(must=conditions) if len(conditions) > 1 else Filter(must=[conditions[0]])
#
#     def _dense_search(
#             self,
#             collection_name: str,
...
#     ) -> List[Dict]:
#         """Pure dense vector search with document filtering"""
#         try:
#             # Encode query
...
#
#             # Search using query_points (correct API for your Qdrant version)
#             kwargs = {
#                 "collection_name": collection_name,
#                 "query": query_vector,  # FIXED: Changed from query_vector to query
#                 "using": "content_vector",  # ADDED: Specify which vector to use
#                 "limit": top_k,
#                 "with_payload": True,
#                 "with_vectors": False
#             }
...
#
#             results = self.client.query_points(**kwargs)  # FIXED: Use query_points instead of search
#
#             chunks = []
#             for pt in (results.points or []):  # FIXED: Access results.points
...
#                 })
#
#             logger.info(f"Dense search returned {len(chunks)} results (score_threshold={score_threshold})")
#             if chunks:
...
#             max_adjacent: int = 1
#     ) -> List[Dict]:
#         """Fetch adjacent chunks - FIXED VERSION"""
#         try:
#             expanded = []
#             chunk_ids_seen = set()
#
...
#
#                 # Fetch adjacent chunks using CORRECT scroll filter
#                 for adj_id in adjacent_ids[:max_adjacent]:
#                     try:
#                         # FIXED: Use proper Filter object
...
#                                     'id': str(pt.id),
#                                     'text': payload.get('text', ''),
#                                     'score': chunk['score'] * 0.95,  # Slightly lower
#                                     'metadata': payload,
...
#
#             logger.info(f"Expanded from {len(chunks)} to {len(expanded)} chunks")
#             return expanded
#
#         except Exception as e:
...
#     ) -> List[Dict]:
#         """
#         Main adaptive retrieval - FIXED VERSION
#
#         Strategy:
#         1. Dense search with document filtering
#         2. Keyword boost for exact matches
#         3. Expand with adjacent chunks
#         4. Multiple fallback attempts if needed
#         """
...
#         )
#
#         # Strategy 2: Fallback with no threshold
#         if not chunks:
#             logger.warning("Initial search returned 0 results, trying without threshold")
#             chunks = self._dense_search(
#                 collection_name, query, profile_id, top_k,
#                 document_ids=document_ids,
#                 source_files=source_files,
...
#             logger.warning("No results with document filter, trying without filter")
#             chunks = self._dense_search(
...
#
#         if not chunks:
#             logger.error("All retrieval strategies failed - no chunks returned")
#             return []
#
#         # Keyword boosting
...
#
#         logger.info(f"Final adaptive retrieval: {len(chunks)} chunks")
#         if chunks:
#             logger.info(f"Top 5 scores: {[round(c['score'], 4) for c in chunks[:5]]}")
#             logger.info(f"Top 3 sources: {[c['metadata'].get('source_file', 'unknown')[:50] for c in chunks[:3]]}")
#
#         return chunks
